...
- The Mac build launches and is signed with Developer ID for team AH277897AV, hardened runtime, no debugger entitlement, and arm64/x86_64 architectures. Local download: ~/Downloads/Pigeonpost-Desktop-1.0-30.zip. Apple accepted notarization submission d389d4e1-7a9f-4047-a796-3ca34a05e7ea (workflow 34578109585); the downloaded final archive passes codesign, stapler validation and Gatekeeper assessment as Notarized Developer ID. Final ZIP SHA-256: 551898e7ddf71bbe0afa383a274601251ff6af7e8fa64462fd2840ff15ab7db3.
...
- Full Pigeonpost CI 34576394115 passed, including Linux/macOS/Windows tests and the Windows executable lifecycle checks. PR https://github.com/bekirdag/pigeonpost/pull/4 merged to main 8886f70. The app source tree is identical to the iOS upload commit 486d16f; subsequent changes only concern CI, a test fixture and release records.
- Local main and both /opt/pigeonpost-src production checkouts were verified clean at the merged implementation commit using checksum-verified incremental bundles. The production Rust source/configuration did not change, so those running services did not need rebuilding or restarting. All Pigeonpost containers remain up, the Postbox is healthy, and logs remain capped at 10m x3. IAM local/production main remains clean and synced at b28c36b; the temporary IAM implementation worktree was removed.

## Remaining verification
Implementation, IAM deployment, both builds, Mac notarization and internal TestFlight distribution are complete. The remaining manual verification is a complete Apple-account consent/sign-in when a browser/user session is available; the live provider redirects and credential exchange were checked without claiming a real-user login.
