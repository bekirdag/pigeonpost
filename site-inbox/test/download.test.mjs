...
const macArchive = "https://github.com/bekirdag/pigeonpost/releases/download/macos-1.0-30/Pigeonpost-Desktop-1.0-30.zip";
...
function platformChoices(document) {
  for (const platform of ["mac", "windows", "linux"]) {
    ...
    assert.equal(action.href, platform === "mac" ? macArchive : webInbox);
  ...
  assert.match(document.querySelector('[data-platform="linux"]').textContent, /Not yet available/);
}
...
for (const [name, navigator, icon] of [
  ...
  ["Linux", { platform: "Linux x86_64", userAgent: "Mozilla/5.0 (X11; Linux x86_64)" }, "linux"],
...
for (const [hint, icon, href] of [["Windows", "windows", webInbox], ["Linux", "linux", webInbox], ["macOS", "mac", macArchive]]) {
