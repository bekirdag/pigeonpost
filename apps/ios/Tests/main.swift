...
check(PeerFace.validConversationAddress("/bekir"), "a namespace can start a conversation")
check(PeerFace.validConversationAddress("/alex+tag@gmail.com"), "email-style handles remain usable")
check(PeerFace.validConversationAddress("/alex*tag@gmail.com/main"), "email punctuation survives the typing guard")
check(!PeerFace.validConversationAddress(PeerFace.conversationAddressInput("")), "a bare slash is not a destination")
check(!PeerFace.validConversationAddress("//bekir"), "duplicate prefix is not a destination")
check(!PeerFace.validConversationAddress("/bekir/../alp"), "traversal is not a destination")
check(!PeerFace.validConversationAddress("/bekir\n"), "line breaks are not part of an address")
